defmodule EmberchatWeb.UserAuthTest do
  use EmberchatWeb.ConnCase

  alias Phoenix.LiveView
  alias Emberchat.Accounts
  alias Emberchat.Accounts.Scope
  alias EmberchatWeb.UserAuth

  import Emberchat.AccountsFixtures
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, EmberchatWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      EmberchatWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        EmberchatWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
